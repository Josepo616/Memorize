...
class GameViewModel: ObservableObject {
    ...
    init(themeViewModel: ThemeViewModel, themeID: UUID) {
        guard themeViewModel.getThemeById(themeID) != nil else {
            fatalError(
                "Theme with ID \(themeID) not found. Check your ThemeCatalog."
            )
        }
    ...
    static func createMemoryGame(with theme: ThemeModel) -> MemoryGame<String> {
        return MemoryGame(numberOfPairOfCards: theme.numberOfPair) {
            pairIndex in
            theme.emojiElements.indices.contains(pairIndex)
                ? theme.emojiElements[pairIndex] : "!?"
        }
    }
